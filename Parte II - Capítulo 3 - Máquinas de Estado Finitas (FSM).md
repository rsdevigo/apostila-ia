...
**O pacote Unity Behavior.** Mais recentemente (a partir de 2024), a Unity passou a oferecer o pacote **Unity Behavior**, uma ferramenta voltada à autoria de comportamento de agentes por meio de grafos. Embora seu foco principal sejam as *árvores de comportamento* (que estudaremos no Capítulo 6), ele também acomoda a lógica de estados e representa a direção oficial da engine para autoria visual de IA. Voltaremos a ele em profundidade no Capítulo 6; aqui basta registrar que a Unity oferece um caminho oficial e visual para construir a decisão dos agentes.
